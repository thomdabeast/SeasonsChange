...
function stopAuto() {
	for (var j = snowflakes.length - 1; j >= 0; j--) {
		$(snowflakes[j]).stop(true, true);
		
	}
	
	while(snowflakes.length > 0){
		document.body.removeChild(snowflakes.pop());
	}
...
function continuousFall() {
	for (var j = snowflakes.length - 1; j >= 0; j--) {
		if (j % 3 == 0) {
			snowAnimation($(snowflakes[j]), j, 1000, 9000);
		}
		else if (j % 3 == 1) {
			snowAnimation($(snowflakes[j]), j, 3000, 9000);
		}
		else{
			snowAnimation($(snowflakes[j]), j, 5000, 9000);
		}
	}
}
